...
``pd.core.series.Series`` is very similar to ``pd.DataFrame`` and shares many attributes.
For instance, we can describe an individual column:
...
useful attributes of ``pd.core.series.Series`` by filtering out methods and attributes that
start with an underscore since they are often builtin or private:
...
#     map(
#       lambda attr: f"- {attr}", 
#       filter(lambda attr: not attr.startswith("_"), dir(DISTANCE))
#     )
#   ), 
# )
```
...
It is easy to use sklearn to attempt to predict the prices of the first ten 
houses:
...
and caclulate the total magnitude of error column using the $L^2$ norm:
...
which implies that an upper bound for the error is 
