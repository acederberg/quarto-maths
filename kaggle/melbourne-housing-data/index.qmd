...
DATA_FEATURES, TEST_FEATURES, DATA_TARGET, TEST_TARGET = train_test_split(
...
Conveniently, the functions above can be used for our out sample data. 
...
TEST_PRICE_COMPARE_ANALYSIS = create_price_compare_analysis(TEST_PRICE_COMPARE)

TEST_PRICE_COMPARE["error_percent"].describe()
```

So far, the model would appear to be inaccurate at best and wildly inaccurate
worst. The following is not encouraging either:


```{python}
#| echo: false
#| fig-cap: Price comparison error analysis.
TEST_PRICE_COMPARE_ANALYSIS
```


This implies that we should probably not use the model to predict null prices
as about 60% of the test data set can be predicted within 20%. However, it 
is possible to better train the model.
