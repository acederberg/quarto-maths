...
err_dist.figure.savefig("./err-dist-in-sample.png")
plt.close()
```
...
Nest, we can createa plot of the count per percent error:


```{python}
def mnae(price_compare: pd.DataFrame):
  return sum(cpc := abs(price_compare["error_percent"])) / (100 * len(cpc))


plt.axvline(mnae(TEST_PRICE_COMPARE) * 100)
err_dist = create_model_errdist(TEST_PRICE_COMPARE, color="red")
err_dist.set_title("Model Out Sample Error Distribution")
err_dist.figure.savefig(DIR / "err-dist-out-sample.png")
plt.close()
```
...
mean normalized absoluted error (as a percentage).
...
sample data, this would not be appearent.
...
```{python}
def create_model_errdist_many(err_dists: list[pd.DataFrame], labels: Sequence[str]):
...
err_dist.figure.savefig("./err-dist-out-sample-many.png")
plt.close()
```
...
same node each time, an inherant problem with decision trees. In the next 
...
and resolve overfitting and underfitting with some sort of consensus many trees
...
err_dist.figure.savefig("./err-dist-forest.png")
