---
title: Zigzag
description: Efficiently solving the 'zigzag'-ification of a string.
filters:
  - include-code-files
---
...
```{.python include="zigzag.py" snippet="solution"}
```
...
3. The ``yield from`` statements could be moved within the outer ``for`` loop
   however this puts more logic within the foor loop and has no performance
   benefit.


The time complexity is linear for a fixed numer of rows and linear for a fixed
...
```{.python include="zigzag.py" snippet="solution_better"}
```
