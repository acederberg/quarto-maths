...
the item $[20, 30]$ satifies that $b_0 = 20 \leq a = 20 \leq 30 = a_1$. 

When an item does fit it should not satify the equality, but instead its 
...
evaulating this statement clearly has linear run time, which is less than 
desirable. 
...
which implies that $[a, b]$ is internally schedulable. Conversly, $k < j$ 
...
1. (Externally schedulable) If the end of the event is less than the earliest 
   scheduled event, add the event to the begining and return ``True``. 
...
the range to be searched in the monotonic sequence $\{a_n\}_{n\inS}$. 
...
Let $N = |S|$. On the zeroeth iteration the numer of points to inspect is $N$,
